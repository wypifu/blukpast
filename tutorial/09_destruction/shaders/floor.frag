...
layout(location = 5) in vec4 vClipPos;
layout(location = 6) in vec4 vReflectPos;
...
layout(set = 0, binding = 3) uniform sampler2D        reflectionMap;
...
void main()
{
    ...
    vec3  F   = FresnelSchlickRGB(max(dot(H, V), 0.0), F0);
    ...
    vec3 base    = ambient + Lo;

    /* Schlick Fresnel for dielectric surface (F0 ≈ 0.02 for tile/water/glass) */
    float NdotV      = abs(dot(N, V));
    float fresnel    = 0.02 + 0.98 * pow(1.0 - NdotV, 5.0);
    float reflAmount = fresnel * (1.0 - roughness);

    /* 0.0 = pure screen-space (turns with camera, objects may drift)
     * 1.0 = pure world-space (anchored, too static)
     * ~0.3 gives a water-like feel: view-dependent but objects stay near their reflections */
    const float REFLECT_BLEND = 0.99;
    vec2 ssUV = (vClipPos.xy    / vClipPos.w)    * 0.5 + 0.5;
    vec2 wsUV = (vReflectPos.xy / vReflectPos.w) * 0.5 + 0.5;
    vec2 screenUV = mix(ssUV, wsUV, REFLECT_BLEND);
    vec4 refl     = texture(reflectionMap, screenUV);

    vec3 color = mix(base, refl.rgb, reflAmount);
